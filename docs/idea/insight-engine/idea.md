...
11. **Agent-based orchestration**: Specialized agents (strategist, audit, advisory) handle different aspects of the pipeline — each optimized for its role rather than one-size-fits-all skill instructions
...
11. **Điều phối bằng agent chuyên biệt**: Các agent (strategist, audit, advisory) xử lý từng khâu — tối ưu cho vai trò riêng thay vì skill chung chung
...
- Specialized agents: strategist (builds dynamic workflow), audit (quality gates), advisory (decision support)
- Shared context file (`tmp/.agent-context.json`) for inter-agent communication (stateless subagents)
- Model self-awareness via self-declaration + decision maps (NO hardcoded model name — users have different Copilot plans)
- Decision maps per capability category: context_window, reasoning_depth, tool_use, multilingual, code_generation — each with 3 levels (basic/standard/advanced)
- Fallback to conservative/medium profile if model cannot self-identify
- Tiered audit: self-review (all steps) → agent audit (critical steps) → final audit (output vs requirements)
- Advisory agent: single-call multi-perspective analysis (max 2 calls/pipeline), only ask user when agent cannot resolve
...
**Kiến trúc agent (Phase 6):**
- Agent chuyên biệt: strategist (tạo workflow), audit (kiểm tra chất lượng), advisory (tư vấn quyết định)
- File context chia sẻ (`tmp/.agent-context.json`) cho giao tiếp giữa các agent
- Model tự nhận diện qua self-declaration + bản đồ quyết định (KHÔNG hardcode model name — user có nhiều plan Copilot khác nhau)
- Bản đồ quyết định theo từng category năng lực: context_window, reasoning_depth, tool_use, multilingual, code_generation — mỗi category 3 mức (basic/standard/advanced)
- Fallback profile trung bình nếu model không tự nhận diện được
- Audit phân tầng: self-review (mọi step) → agent audit (step quan trọng) → final audit (output vs yêu cầu)
- Agent advisory: phân tích đa góc nhìn trong 1 lần gọi (max 2 lần/pipeline), chỉ hỏi user khi agent không giải quyết được
